...
When `DICTATION_BACKEND` is `"none"` (the Linux default), the Up button is repurposed as a stateless quick-action button instead of a dictation toggle: each press types `config.QUICK_ACTION` (`FLIPPER_QUICK_ACTION`, default `/compact`) into the focused terminal and presses Enter (`daemon.py`'s `voice` handler). Configure a different command via the env var, the plugin's `quickAction` userConfig option, or directly on the Flipper (**Right-hold → MENU → Quick Action**, Bridge mode only — `ui.c`'s `INFO_IDX_QUICKACTION` opens a `TextInput` view, persisted via `app_settings_get/set_quick_action`). The on-device value is announced to the host in every `hello` and immediately on save via a new `qa_set` message (`daemon.py`'s `_apply_device_quick_action`, overrides `config.QUICK_ACTION` at runtime); the host echoes the value actually in effect back to the Flipper's header on every `ping` (`"qa"` field, `ui_set_quick_action`/`status_draw` in `ui.c`) so the header always shows what Up will actually send. Leaving the on-device field blank reverts to the host default.
